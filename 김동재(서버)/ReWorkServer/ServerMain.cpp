...
private:
	...
	void Send_Packet(void* packet, unsigned id)
	{
		...
		players[id].load()->do_write(buff, packet_size);
	}

	void Process_Packet(unsigned char* packet, int id)
	{
		auto P = players[id].load();
		int y = P->pos_y;
		int x = P->pos_x;
		switch (packet[1]) {
...
public:
	...
	void start()
	{
		...
		for (auto& pl : players)
			if (pl.second.load() != nullptr)
				pl.second.load()->Send_Packet(&p);
		for (auto& pl : players) {
			if (pl.second.load()->my_id_ != my_id_) {
				p.id = pl.second.load()->my_id_;
				p.x = pl.second.load()->pos_x;
				p.y = pl.second.load()->pos_y;
				Send_Packet(&p);
			}
		}
	}
