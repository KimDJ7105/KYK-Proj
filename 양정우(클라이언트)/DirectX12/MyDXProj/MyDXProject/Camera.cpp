...
CCamera::CCamera()
{
	...
	// 카메라 변환 행렬
	m_xmf4x4View = Matrix4x4::Identity();												// 단위 행렬로 초기화
	// 투영 변환 행렬
	m_xmf4x4Projection = Matrix4x4::Identity();											// 단위 행렬로 초기화

	// 뷰포트와 씨저 사각형
	m_d3dViewport = { 0, 0, FRAME_BUFFER_WIDTH , FRAME_BUFFER_HEIGHT, 0.0f, 1.0f };		// Viewport 초기화
	m_d3dScissorRect = { 0, 0, FRAME_BUFFER_WIDTH , FRAME_BUFFER_HEIGHT };				// ScissorRect 초기화
...
{
	if (pCamera)																		// 카메라가 이미 있으면
	{
		*this = *pCamera;																// 기존 카메라의 정보를 새로운 카메라에 복사한다.
	}
	else
	{
...
void CCamera::UpdateShaderVariables(ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	XMStoreFloat4x4(&xmf4x4View, XMMatrixTranspose(XMLoadFloat4x4(&m_xmf4x4View)));					// Camera View Matrix 저장

	//루트 파라메터 인덱스 1의
	pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4View, 0);							// Command List에 해당 내용 업데이트

	XMFLOAT4X4 xmf4x4Projection;
	XMStoreFloat4x4(&xmf4x4Projection, XMMatrixTranspose(XMLoadFloat4x4(&m_xmf4x4Projection)));		// Projection View Matrix 저장

	pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4Projection, 16);					// Command List에 해당 내용 업데이트
}
...
{
	// 카메라 변환 행렬을 생성한다.
	// 카메라의 위치 벡터, 카메라가 바라보는 지점, 카메라의 Up 벡터(로컬 y-축 벡터)를 파라메터로 사용하는 XMMatrixLookAtLH() 함수를 사용한다.
...
void CCamera::RegenerateViewMatrix()
{
	...
	// 카메라 변환 행렬이 바뀔 때마다 절두체를 다시 생성한다
	// (절두체는 월드 좌표계로 생성한다)
	GenerateFrustum();
}
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		if (pCamera->GetMode() == SPACESHIP_CAMERA)					// FIRST_PERSON_CAMERA로 변경하기 이전의 카메라가 SPACESHIP_CAMERA 카메라이면
		{
			m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);					// 카메라의 Up 벡터를 월드좌표의 y-축이 되도록 한다.
			m_xmf3Right.y = 0.0f;									// 로컬 x-축 벡터와
			m_xmf3Look.y = 0.0f;									// 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다
			m_xmf3Right = Vector3::Normalize(m_xmf3Right);			// 1인칭 카메라의 로컬 x-축 벡터와 
			m_xmf3Look = Vector3::Normalize(m_xmf3Look);			// 로컬 z-축 벡터는 xz-평면에 평행하다
		}
	}
}
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		if (pCamera->GetMode() == SPACESHIP_CAMERA)					// THIRD_PERSON_CAMERA로 변경하기 이전의 Mode가 SPACESHIP_CAMERA 이면
		{
			m_xmf3Up = XMFLOAT3(0.0f, 1.0f, 0.0f);					// 카메라의 Up 벡터를 월드좌표의 y-축이 되도록 한다.
			m_xmf3Right.y = 0.0f;									// 로컬 x-축 벡터와
			m_xmf3Look.y = 0.0f;									// 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다.
			m_xmf3Right = Vector3::Normalize(m_xmf3Right);			// 3인칭 카메라의 로컬 x-축 벡터와 
			m_xmf3Look = Vector3::Normalize(m_xmf3Look);			// 로컬 z-축 벡터는 xz-평면에 평행하다.
		}
	}
}
...
void CThirdPersonCamera::Update(XMFLOAT3& xmf3LookAt, float fTimeElapsed)
{
	...
	if (m_pPlayer)
	{
		...
		// Lag - (역동적인 카메라워크!, m_fTimeLag가 1보다 크면 화면이동이 적게 나옴)
