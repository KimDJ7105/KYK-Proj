...
{
	// bUpdateVelocity가 참이면 
	if (bUpdateVelocity)
	{
		// 플레이어를 이동하지 않고 속도 벡터를 변경한다.
		m_xmf3Velocity = Vector3::Add(m_xmf3Velocity, xmf3Shift);	// 플레이어의 속도 벡터를 xmf3Shift 벡터만큼 변경한다.
	}
	else
	{
		//플레이어를 현재 위치 벡터에서 xmf3Shift 벡터만큼 이동한다. 
...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	//FIRST_PERSON_CAMERA 또는 THIRD_PERSON_CAMERA의 경우 플레이어의 회전은 약간의 제약이 따른다.
	...
	{
		// SPACESHIP_CAMERA 카메라에서 플레이어의 회전은 회전 각도의 제한이 없다.
...
void CPlayer::Update(float fTimeElapsed)
{
	...
	//이것이 XZ-평면의 최대 속력보다 크면 속도 벡터의 x와 z-방향성분을 조정한다.
	...
	//Move(xmf3Velocity, false);		// 요거 잘못됐음
	...
	float fDeceleration = (m_fFriction * fTimeElapsed);									// 마찰 계수를 시간에 비례하도록 하여 마찰력을 구한다.
	if (fDeceleration > fLength) fDeceleration = fLength;								// 마찰력이 속력보다 크면 속력은 0이 될 것이다.

	m_xmf3Velocity = Vector3::Add(m_xmf3Velocity, 
		Vector3::ScalarProduct(m_xmf3Velocity, -fDeceleration, true)					// 단위 벡터에 마찰력을 곱하여 감속 벡터를 구한다.
...
CCamera* CPlayer::OnChangeCamera(DWORD nNewCameraMode, DWORD nCurrentCameraMode)
{
	...
	if (nCurrentCameraMode == SPACESHIP_CAMERA)											// 현재 카메라의 모드가 SPACESHIP_CAMERA 모드의 카메라이고 새로운 카메라가 FIRST_PERSON_CAMERA 또는 THIRD_PERSON_CAMERA이면
	{
		...
		m_fPitch = 0.0f;																// 그러므로 플레이어의 Right 벡터와 Look 벡터의 y 값을 0으로 만든다
		m_fRoll = 0.0f;																	// 그러므로 플레이어의 Right 벡터와 Look 벡터의 y 값을 0으로 만든다

		m_xmf3Right = Vector3::Normalize(XMFLOAT3(m_xmf3Right.x, 0.0f, m_xmf3Right.z));	
		m_xmf3Up = Vector3::Normalize(XMFLOAT3(0.0f, 1.0f, 0.0f));						// 플레이어의 Up 벡터를 월드좌표계의 y - 축 방향 벡터(0, 1, 0)이 되도록 한다 (즉, 똑바로 서도록 한다)
...
CCamera* CAirplanePlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case SPACESHIP_CAMERA:			//플레이어의 특성을 스페이스-쉽 카메라 모드에 맞게 변경한다. 중력은 적용하지 않는다.
		SetFriction(125.0f);
	...
	case THIRD_PERSON_CAMERA:		//플레이어의 특성을 3인칭 카메라 모드에 맞게 변경한다.
		...
		//값을 0.25f 대신에 0.0f와 1.0f로 설정한 결과를 비교하기 바란다.
